...
*/
int main(){
 int opcao, mmr, tier, vitorias, derrotas, total_de_partidas;
 float winrate_total;
 char input_winrate;
 ...
 switch(opcao){
   case 'a':
    ...
    break;
  case 'b':
     printf("Você escolheu a taxa de vitória\n");
     do {
      printf("Digite o resultado da Partida (V/D): ");
      scanf("%c", &input_winrate);
    } while(input_winrate == 'V' || input_winrate == 'D');
      if(input_winrate)
      break;
    default:
     break;
